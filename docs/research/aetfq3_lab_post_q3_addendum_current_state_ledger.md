...
- this closeout commit: group-level diagnostic smoke model-signal review closeout and ledger update.
...
| F-public | `F_PUBLIC_STATUS=GROUP_LEVEL_DIAGNOSTIC_SMOKE_COMPLETED_MODEL_SIGNAL_REVIEW_REQUIRED_NO_FORMAL_EVIDENCE` | `aetfq3_intraday_public_no_label_tensor_validation.*`; `aetfq3_intraday_label_outcome_design.*`; `aetfq3_intraday_label_manifest_leakage_checker.*`; `aetfq3_intraday_label_generation_intake_orchestrator.*`; `aetfq3_intraday_label_generation_pause_closeout.*`; `aetfq3_intraday_supervised_smoke_readiness_precheck.*`; `aetfq3_intraday_supervised_no_save_smoke.*`; `aetfq3_intraday_supervised_no_save_smoke_review_closeout.*`; `aetfq3_intraday_supervised_no_save_repeatability_check.*`; `aetfq3_intraday_larger_eligible_anchor_smoke_repeatability_closeout.*`; `aetfq3_intraday_group_level_sample_dryrun.*`; `aetfq3_intraday_group_level_supervised_smoke_readiness_precheck.*`; `aetfq3_intraday_group_level_no_save_diagnostic_smoke.*`; `aetfq3_intraday_group_level_model_signal_review_closeout.*`; ignored group-level diagnostic smoke reports | Group-level no-save diagnostic smoke is complete, but ordinary logistic still collapsed to the majority-class dummy; balanced/scaled probe reduced collapse but balanced accuracy remained below 0.5, so there is no formal model evidence. P1 label inconsistency review remains required. | Human review of label inconsistency, past-only feature expansion design, group-level feature diagnostic design, or optional no-save diagnostic smoke with separate task card | Review-only diagnostic follow-up within Lab boundary | Stable promotion, QMT, OrderIntent, advisory, formal training, model deployment, checkpoint/model save |
...
- F public group-level diagnostic smoke completed: ordinary logistic still matched the majority-class dummy, balanced/scaled probe reduced collapse, balanced accuracy remained below 0.5, and no formal model evidence exists. P1 label inconsistency review remains required.
...
For the completed F-public group-level no-save diagnostic smoke, only review follow-ups are allowed: human review of label inconsistency, past-only feature expansion design, group-level feature diagnostic design, or optional no-save diagnostic smoke with a separate task card. Stable promotion, formal training, QMT, OrderIntent, advisory, and model deployment remain forbidden.
...
- `F_PUBLIC_STATUS=GROUP_LEVEL_DIAGNOSTIC_SMOKE_COMPLETED_MODEL_SIGNAL_REVIEW_REQUIRED_NO_FORMAL_EVIDENCE`
