...
| F-public | `F_PUBLIC_STATUS=LAB_MONITOR_CANDIDATE_REVIEW_READY_ROLLING_ORIGIN_STABILITY_OBSERVED_STABLE_BLOCKED` | `aetfq3_intraday_public_no_label_tensor_validation.*`; `aetfq3_intraday_label_outcome_design.*`; `aetfq3_intraday_label_manifest_leakage_checker.*`; `aetfq3_intraday_label_generation_intake_orchestrator.*`; `aetfq3_intraday_label_generation_pause_closeout.*`; `aetfq3_intraday_supervised_smoke_readiness_precheck.*`; `aetfq3_intraday_supervised_no_save_smoke.*`; `aetfq3_intraday_supervised_no_save_repeatability_check.*`; `aetfq3_intraday_group_level_transform_aware_no_save_smoke.*`; `aetfq3_intraday_rolling_oop_pool_capture.*`; `aetfq3_intraday_historical_5m_manual_intake_closeout.*`; `aetfq3_intraday_fixed_shortlist_oop_no_save_validation.*`; `aetfq3_intraday_oop_post_sprint_reversal_attribution.*`; `aetfq3_intraday_rolling_origin_walk_forward_no_save_validation.*`; `aetfq3_intraday_rolling_origin_stability_attribution_review.*`; `aetfq3_intraday_lab_monitor_candidate_protocol.*`; `aetfq3_intraday_lab_monitor_candidate_closeout.*`; ignored Lab monitor candidate status report | A unique Lab-only monitor candidate is review-ready after rolling-origin stability attribution, but it is not Stable evidence. Post-sprint forward-only evidence remains underpowered and the monitor candidate cannot trigger Stable, QMT, OrderIntent, advisory, formal training, checkpoint/model/scaler save, or automatic promotion. | Periodic Lab-only monitor refresh with new compliant manual 5m data, followed by read-only status compiler review | Lab-only monitor protocol / periodic data refresh; no-save read-only diagnostics; human review of continuation/review/retire gates | Stable promotion, QMT, OrderIntent, advisory package, formal training, model deployment, checkpoint/model/scaler save, automatic promotion, BUY/PROBE threshold change, `target_weight` change, or `final_buy_action` change |
...
5. A Lab-only periodic monitor refresh is explicitly requested with compliant data and read-only boundaries.
...
- `F_PUBLIC_STATUS=LAB_MONITOR_CANDIDATE_REVIEW_READY_ROLLING_ORIGIN_STABILITY_OBSERVED_STABLE_BLOCKED`
