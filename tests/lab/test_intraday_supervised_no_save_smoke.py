...
OUT_ROOT = Path(".local_research_outputs/aetfq3_lab/intraday_supervised_no_save_smoke/pytest")
FEATURE_COLUMNS = [
    "open",
    "high",
    "low",
...
def test_smoke_cli_on_fixture_succeeds(tmp_path: Path, capsys) -> None:
    manifest = write_manifest(tmp_path)
    readiness = write_readiness(tmp_path)
    out_dir = OUT_ROOT / "cli_success"

    exit_code = main(["--samples", str(SAMPLES), "--manifest", str(manifest), "--readiness", str(readiness), "--out-dir", str(out_dir)])
...
def test_readiness_not_passed_blocks(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "readiness_blocked")
...
def test_label_in_feature_columns_blocks(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "label_feature")
...
def test_outcome_in_feature_columns_blocks(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "outcome_feature")
...
def test_training_allowed_true_blocks_boundary(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "training_allowed")
...
def test_contains_order_intent_true_blocks_boundary(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "order_intent")
...
def test_output_artifact_check_catches_pkl_and_pt(tmp_path: Path) -> None:
    ...
    report = run_smoke(SAMPLES, manifest, readiness, OUT_ROOT / "artifact_blocked")
...
def test_report_json_includes_no_save_boundary_fields(tmp_path: Path) -> None:
    manifest = write_manifest(tmp_path)
    readiness = write_readiness(tmp_path)
    out_dir = OUT_ROOT / "report_fields"

    report = run_smoke(SAMPLES, manifest, readiness, out_dir)
    payload = json.loads((REPO_ROOT / out_dir / "intraday_supervised_no_save_smoke_report.json").read_text(encoding="utf-8"))
